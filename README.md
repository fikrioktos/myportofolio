...
1. **Jelaskan mengapa kita menggunakan ModelForm pada Django alih-alih membuat form HTML secara manual. Selain itu, jelaskan pula mengapa kita diwajibkan menambahkan {% csrf_token %} pada form tersebut!**
   ...
   `ModelForm` menghilangkan dua pekerjaan yang harus dilakukan dua kali kalau form dibuat manual. Pertama, definisi field: dengan `ExperienceForm(forms.ModelForm)` di `main/forms.py`, setiap field form otomatis lahir dari model `Experience`, lengkap dengan tipe dan aturan validasinya. Field `started_at` yang di model bertipe `DateField` otomatis menjadi input tanggal di form tanpa saya menulis ulang tipenya. Kedua, validasi: aturan seperti `max_length` dan sifat wajib sebuah field divalidasi di satu tempat yang sama, sehingga tidak mungkin aturan di model dan di HTML form saling bertentangan.

   Konsekuensi praktisnya terasa di proyek ini. Karena form dibangun dari model, menambah satu field baru cukup mengubah `models.py` lalu membuat migrasi, dan form ikut berubah sendiri. Saya juga bebas menambahkan validasi yang tidak dimiliki model melalui method `clean()`, misalnya `DateRangeFormMixin` yang menolak tanggal selesai lebih awal dari tanggal mulai, serta mixin `AccessCodeFormMixin` yang memvalidasi kode akses langsung di level form. Validasi di form (bukan hanya di view) penting karena request POST bisa datang langsung dari curl atau test client tanpa pernah melewati HTML form, dan tetap harus ditolak dengan benar.

   Untuk `{% csrf_token %}`, Django melindungi setiap request POST dengan Cross Site Request Forgery protection. Tanpa token tersebut, situs jahat bisa menyelipkan form tersembunyi yang mengirimkan POST ke endpoint create atau delete portofolio saya memakai sesi browser pengunjung yang sedang login, dan server tidak punya cara membedakannya dari submit yang sah. Tag tersebut menanamkan token unik di dalam form, dan setiap POST yang datang tanpa token yang cocok akan ditolak dengan error 403. Pada implementasinya, token ini juga menjadi alasan `CSRF_TRUSTED_ORIGINS` perlu diisi dengan URL PWS, karena request POST dari domain production juga harus lolos pemeriksaan yang sama.

2. **Pada Tutorial 03, kita membahas format data JSON dan XML. Mengapa JSON lebih disukai dalam pengembangan aplikasi web modern dibandingkan XML?**

   Alasan utamanya adalah hubungan JSON dengan JavaScript. Aplikasi web modern mengambil data lewat `fetch()` dan langsung memakainya di browser, dan JSON adalah representasi objek JavaScript itu sendiri, jadi hasil response tidak perlu di-parse khusus. XML harus diparse lebih dulu ke dalam struktur DOM sebelum bisa dibaca, yang lebih boros dan lebih verbose.

   Kedua, JSON lebih ringkas. Untuk data yang sama, XML membawa banyak karakter untuk tag pembuka dan penutup yang tidak membawa informasi tambahan, misalnya `<title>...</title>` berulang untuk setiap entri experience. JSON menuliskan pasangan key-value yang sama dalam bentuk `"title": "..."`, sehingga payload lebih kecil dan lebih murah dikirim. Struktur JSON juga lebih sederhana: hanya ada object, array, string, number, boolean, dan null, sementara XML punya namespace, atribut versus elemen teks, dan aturan lain yang sering tidak perlu untuk data delivery.

   Meskipun begitu, JSON bukan pengganti mutlak XML di semua konteks. XML masih dipakai untuk standar lama seperti SOAP dan dokumen yang butuh struktur campuran teks dan markup. Di Tutorial 03 saya tetap membangun endpoint XML (`/api/experience/xml/` dan `/api/projects/xml/`) bersebelahan dengan versi JSON-nya, dan saya melihat langsung perbedaannya: isi datanya identik karena keduanya dihasilkan dari queryset yang sama melalui `serializers.serialize` yang sama, yang beda hanya format keluarannya. JSON unggul untuk kebutuhan API web modern, dan itulah sebabnya format ini yang menjadi pilihan default.

3. **Jelaskan alur yang terjadi saat kamu menggunakan fungsi view untuk mengembalikan data portofoliomu dalam bentuk JSON. Mengapa kita perlu melakukan proses serialization pada model Django sebelum datanya dikembalikan?**

   Alurnya bisa dilacak dari `get_experience_json` di `main/views.py`. View memulai dengan queryset `Experience.objects.all().order_by("-started_at")`, lalu queryset itu diteruskan ke `serializers.serialize("json", ...)`. Fungsi ini mengubah setiap objek model menjadi dictionary JSON yang berisi primary key, nama model, dan nilai setiap field. Hasil string JSON tersebut dikemas dalam `HttpResponse` dengan `content_type="application/json"`, sehingga siap dikonsumsi siapa pun yang meminta endpoint tersebut. Alur serupa berlaku untuk `get_projects_json`, hanya saja lewat helper `_projects_matching_query` karena endpoint itu juga mendukung filter `?title=`.
